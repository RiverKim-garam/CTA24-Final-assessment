...
```{r}
# Total sentiment of text including immigration related words plot
nrc_data %>%
...
golve_embedding <- readRDS(url(url, method = "libcurl"))
```
...
ggplot(selected_words, aes(x = UMAP1, y = UMAP2)) + 
  ...
  labs(title = "GloVe word embedding of words related to 'immigration'") +
...
ggplot(selected_words_2, aes(x = UMAP1, y = UMAP2)) + 
  geom_point(show.legend = FALSE) + 
  geom_text(aes(UMAP1, UMAP2, label = word), show.legend = FALSE, size = 2.5, vjust=-1.5, hjust=0) +
  labs(title = "Total parliament immigration related speech word embedding of 'immigrants'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```

Total parliament immigration related speech word embedding of words related to *asylums*

```{r}
# Plot the word embedding of words that are related for the GloVe model (Case3: asylums)
word_3 <- glove_embedding["asylums",, drop = FALSE]
cos_sim = sim2(x = glove_embedding, y = word_3, method = "cosine", norm = "l2")
select <- data.frame(rownames(as.data.frame(head(sort(cos_sim[,1], decreasing = TRUE), 25))))
colnames(select) <- "word"
selected_words_3 <- df_glove_umap %>% 
  inner_join(y=select, by= "word")
```

```{r}
#The ggplot visual for GloVe
ggplot(selected_words_3, aes(x = UMAP1, y = UMAP2)) + 
  geom_point(show.legend = FALSE) + 
  geom_text(aes(UMAP1, UMAP2, label = word), show.legend = FALSE, size = 2.5, vjust=-1.5, hjust=0) +
  labs(title = "Total parliament immigration related speech word embedding of 'asylums'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```

Total parliament immigration related speech word embedding of words related to *asylum*
...
ggplot(selected_words_4, aes(x = UMAP1, y = UMAP2)) + 
  geom_point(show.legend = FALSE) + 
  geom_text(aes(UMAP1, UMAP2, label = word), show.legend = FALSE, size = 2.5, vjust=-1.5, hjust=0) +
  labs(title = "Total parliament immigration related speech word embedding of 'asylum'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```
...
golve_embedding_pre <- readRDS(url(url_pre, method = "libcurl"))
```
...
ggplot(selected_words_pre_1, aes(x = Pre_UMAP1, y = Pre_UMAP2)) + 
  ...
  labs(title = "(Pre-Crisis) total parliament speech word embedding of 'immigration'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```
...
ggplot(selected_words_pre_2, aes(x = Pre_UMAP1, y = Pre_UMAP2)) + 
  geom_point(show.legend = FALSE) + 
  geom_text(aes(Pre_UMAP1, Pre_UMAP2, label = word), show.legend = FALSE, size = 2.5, vjust=-1.5, hjust=0) +
  labs(title = "(Pre-Crisis) total parliament speech word embedding of 'immigrants'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```

Pre-crisis total parliament speech word embedding of words related to *asylums*

```{r}
# Plot the word embedding of words that are related for the GloVe model (Case3: asylums)
word_pre_3 <- glove_embedding_pre["asylums",, drop = FALSE]
...
ggplot(selected_words_pre_3, aes(x = Pre_UMAP1, y = Pre_UMAP2)) + 
  geom_point(show.legend = FALSE) + 
  geom_text(aes(Pre_UMAP1, Pre_UMAP2, label = word), show.legend = FALSE, size = 2.5, vjust=-1.5, hjust=0) +
  labs(title = "(Pre-Crisis) total parliament speech word embedding of 'asylums'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```
...
```{r}
# Plot the word embedding of words that are related for the GloVe model (asylum)
word_pre_4 <- glove_embedding_pre["asylum",, drop = FALSE]
cos_sim = sim2(x = glove_embedding_pre, y = word_pre_4, method = "cosine", norm = "l2")
select <- data.frame(rownames(as.data.frame(head(sort(cos_sim[,1], decreasing = TRUE), 25))))
colnames(select) <- "word"
selected_words_pre_4 <- df_umap_pre %>% 
  inner_join(y=select, by= "word")
```

```{r}
#The ggplot visual for GloVe
ggplot(selected_words_pre_4, aes(x = Pre_UMAP1, y = Pre_UMAP2)) + 
  geom_point(show.legend = FALSE) + 
  geom_text(aes(Pre_UMAP1, Pre_UMAP2, label = word), show.legend = FALSE, size = 2.5, vjust=-1.5, hjust=0) +
  labs(title = "(Pre-Crisis) total parliament speech word embedding of 'asylum'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```

### (3) The Glove model for total Parliament speech after the 2008 economic crisis
...
golve_embedding_post <- readRDS(url(url_post, method = "libcurl"))
```
...
ggplot(selected_words_post_1, aes(x = Post_UMAP1, y = Post_UMAP2)) + 
  ...
  labs(title = "(Post-Crisis) total parliament speech word embedding of 'immigration'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```
...
ggplot(selected_words_post_2, aes(x = Post_UMAP1, y = Post_UMAP2)) + 
  geom_point(show.legend = FALSE) + 
  geom_text(aes(Post_UMAP1, Post_UMAP2, label = word), show.legend = FALSE, size = 2.5, vjust=-1.5, hjust=0) +
  labs(title = "(Post-Crisis) total parliament speech word embedding of 'immigrants'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```

Post-crisis total parliament speech word embedding of words related to *asylums*

```{r}
# Plot the word embedding of words that are related for the GloVe model (Case3: asylums)
word_post_3 <- glove_embedding_post["asylums",, drop = FALSE]
cos_sim = sim2(x = glove_embedding_post, y = word_post_3, method = "cosine", norm = "l2")
select <- data.frame(rownames(as.data.frame(head(sort(cos_sim[,1], decreasing = TRUE), 25))))
colnames(select) <- "word"
selected_words_post_3 <- df_umap_post %>% 
  inner_join(y=select, by= "word")
```

```{r}
#The ggplot visual for GloVe
ggplot(selected_words_post_3, aes(x = Post_UMAP1, y = Post_UMAP2)) + 
  geom_point(show.legend = FALSE) + 
  geom_text(aes(Post_UMAP1, Post_UMAP2, label = word), show.legend = FALSE, size = 2.5, vjust=-1.5, hjust=0) +
  labs(title = "(Post-Crisis) total parliament speech word embedding of 'asylums'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```
...
ggplot(selected_words_post_4, aes(x = Post_UMAP1, y = Post_UMAP2)) + 
  geom_point(show.legend = FALSE) + 
  geom_text(aes(Post_UMAP1, Post_UMAP2, label = word), show.legend = FALSE, size = 2.5, vjust=-1.5, hjust=0) +
  labs(title = "(Post-Crisis) total parliament speech word embedding of 'asylum'") +
  theme(plot.title = element_text(hjust = .5, size = 14))
```
